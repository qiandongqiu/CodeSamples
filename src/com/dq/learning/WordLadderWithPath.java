...
public class WordLadderWithPath {
	...
	/**
	 * ladderPath: find the one shortest path from begin word to end word, using
	 * BFS.
	 * 
	 * @param beginWord
	 * @param endWord
	...
	public List<String> ladderPath(String beginWord, String endWord, List<String> wordList) {
		...
		// What gets put in the Q is the PATH to this node, with the current node as the
		// last one
		...
		while (!Q.isEmpty()) {
			...
			for (int i = 0; i < currentSize; i++) {
				List<String> currentPath = Q.remove(0);
				String currentWord = currentPath.remove(currentPath.size() - 1); // last one is the current node
	...
	/**
	 * Find all paths from beginWord to endWord.
	 * 
	 * @param beginWord
	 * @param endWord
	 * @param wordList
	 * @return
	 */
	public List<List<String>> ladderAllPath(String beginWord, String endWord, List<String> wordList) {
	...
	/**
	 * find all shortest paths from beginWord to endWord. Based on the algo above,
	 * since we use BFS, the first one is the shortest; but we may have another one
	 * with the same length; we need to get those. If next path is longer than the
	 * first one, we just return.
	 * 
	 * @param beginWord
	 * @param endWord
	 * @param wordList
	 * @return
	 */
	public List<List<String>> ladderAllShortestPath(String beginWord, String endWord, List<String> wordList) {

		List<List<String>> result = new LinkedList<>();
		int firstShortestPathSize = 0;
		...
		while (!Q.isEmpty()) {
			...
			for (int i = 0; i < currentSize; i++) {
				if(Q.isEmpty()) break;
				List<String> currentPath = Q.remove(0);
				String currentWord = currentPath.remove(currentPath.size() - 1); // the last one is the current node
				//System.out.print(currentWord+" ");
				currentPath.add(currentWord);
				usedWords.add(currentWord);    //add it to set, for faster check later on
				
				if (currentWord.equals(endWord)) {
					if (result.size() == 0) {
						firstShortestPathSize = currentPath.size();
					}
					if (currentPath.size() <= firstShortestPathSize) {
						result.add(currentPath);
						continue;
					} else {
						//we find another path which is longer than the first one; we return, no need to go to next path
                        return result;
					}
				}

							
				//another way: since beginWord is samll, we go through every char in the word, change it to 'a' to 'z'
				char[] currentWordArr = currentWord.toCharArray();
				for(i=0;i<currentWordArr.length; i++) {
					for(char ch='a'; ch<='z'; ch++) {
					  char oldCh = currentWordArr[i];
					  if(oldCh == ch) continue;
					  currentWordArr[i] = ch;
					  String word= new String(currentWordArr);
					  //if( (!currentPath.contains(word)) && (wordList.contains(word))) {  //this line: currentPath.contains(word) is bad
					  //if( (!usedWords.contains(word)) && (wordList.contains(word))) {  //this line: this line solved the TIMEOUT issue!!!
					  if( (!usedWords.contains(word)) && (allWords.contains(word))) {  //this line: this line solved the TIMEOUT issue!!!
	...
	public static void main(String[] args) {
		...
		//This would cause index out of range
		//String beginWord = "cat";
		//String endWord = "fin";
		//String[] words= {"ion","rev","che","ind","lie","wis","oct","ham","jag","ray","nun","ref","wig","jul","ken","mit","eel","paw","per","ola","pat","old","maj","ell","irk","ivy","beg","fan","rap","sun","yak","sat","fit","tom","fin","bug","can","hes","col","pep","tug","ump","arc","fee","lee","ohs","eli","nay","raw","lot","mat","egg","cat","pol","fat","joe","pis","dot","jaw","hat","roe","ada","mac"};
		...
		wordsList.addAll(Arrays.asList(words)); // NOTE: ArrayList does not have remove() defined; remove() is used on
												// line 43;

		WordLadderWithPath wl = new WordLadderWithPath();

		// List<String> result = wl.ladderPath(beginWord, endWord, wordsList);
		// System.out.println(result);

		//List<List<String>> result = wl.ladderAllPath(beginWord, endWord, wordsList);
		List<List<String>> result = wl.ladderAllShortestPath(beginWord, endWord, wordsList);
